...
class FollowWaypointsServer(Node):
    ...
    def execute_callback(self, goal_handle):
        ...
        waypoints = [
            (Point(x=pose_stamped.pose.position.x, y=pose_stamped.pose.position.y, z=0.0))
            for pose_stamped in incoming_poses_stamped
        ]
        feedback = FollowWaypoints_Feedback()
        finished = False
        self.los_current_pub.publish(Point(x=self.position.linear.x, y=self.position.linear.y, z=0.0))
        ...
        def control_vessel():
            ...
            nonlocal finished
            while not finished:
                self.path_pub.publish(Path(header=Header(stamp=self.get_clock().now().to_msg(), frame_id='map'), poses=incoming_poses_stamped[index:]))
                distance = math.sqrt((waypoints[index].x - self.position.linear.x) ** 2
